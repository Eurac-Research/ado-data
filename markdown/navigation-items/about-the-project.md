---
title: About the Project
slug: about-the-project
---


## 1. Background & Motivation  
Droughts are an increasing concern in the Alpine region and in the lowland areas that rely on Alpine water supplies.  
The **Alpine Drought Observatory (ADO)** provides a tool for a quick and easy overview of the current drought situation in the Alps, as well as past drought events over the last 40 years of time-series data.

## 2. The ADO Phase (Monitoring)  
### What was done  
ADO integrates data from climate models, satellite observations and ground-station networks across all Alpine countries.  
It harmonises this information and updates it on a regular basis. Users can access **open-data (CC-BY 4.0)** through the web interface, selecting different drought indices (such as SPI, SPEI, SSPI, vegetation indices, hydrological data) to explore drought conditions by region and over time.

### Partners & Scope  
The ADO project (2019-2022) brought together a consortium of research institutions and national/regional agencies across the Alpine Space.  
It was applied in six case studies across the Alpine countries, addressing agricultural drought, hydrological drought, and ecosystem impacts.

### Key Outputs  
- A web-platform for drought monitoring across the Alpine Space.  
- An inventory of drought impacts (“EDII-Alps”) documenting more than 3,000 recorded events and impacts.  
- Guidelines for drought risk management and recommendations for governance in mountain regions.  

Learn more:  
- [ADO project on Alpine Space](https://www.alpine-space.eu/project/ado/)  
- [EURAC Research project page](https://www.eurac.edu/en/institutes-centers/institute-for-earth-observation/projects/ado)

## 3. The A-DROP Phase (Forecasting & Services)  
### Why the next step  
Building on the successful ADO phase, the work is now being advanced and expanded through the new **A-DROP** project (Alpine DROught Prediction) under the **Interreg Alpine Space 2021-2027** programme.  
The focus shifts from primarily monitoring to **forecasting, operational services**, and tailored sectoral solutions.

### What A-DROP does  
- Develops a **multi-sectoral drought forecasting system** that combines the existing ADO monitoring infrastructure with new datasets, prediction tools and tailored services for sectors such as hydropower, agriculture and winter tourism.  
- Involves 11 partners from five Alpine-Space countries — from research institutions to public administrations.  
- Implements real-life pilot studies (in agriculture, hydropower, snow/water management) and regional climate model simulations to estimate future drought conditions.  
- Offers tailored training and outreach events to ensure the transition from research to operational practice and to support water governance.  

More info: [A-DROP project page](https://www.alpine-space.eu/project/a-drop/)

## 4. What You Can Do / Why It Matters  
Through these two phases—monitoring via **ADO**, and forecasting & services via **A-DROP**—we aim to enhance drought risk management across the Alpine region.  
By moving from **monitoring** (ADO) to **forecasting and operational responses** (A-DROP), we support decision-makers and stakeholders in anticipating, preparing for and adapting to droughts in mountain and downstream regions.

For users this means:  
- Access to open data and drought indicators for the Alpine Space (ADO)  
- Upcoming or developing services, tools and tailored applications for specific sectors (A-DROP)  
- A stronger evidence base (historical impacts, vulnerabilities) and improved capability to plan ahead  

## 5. Quick Links  
- Visit the ADO platform: [https://ado.eurac.edu/](https://ado.eurac.edu/)  
- Learn more about A-DROP: [https://www.alpine-space.eu/project/a-drop/](https://www.alpine-space.eu/project/a-drop/)

### 6. Partners of A-DROP

The A-DROP consortium brings together **11 partners from five Alpine Space countries**, combining expertise in research, monitoring, and operational water management.

- **Eurac Research** (Lead Partner) – Bolzano/Bozen, Italy  
  [www.eurac.edu](https://www.eurac.edu)

- **Slovene Chamber of Agriculture and Forestry – Institute of Agriculture and Forestry Maribor** – Maribor, Slovenia  
  [www.kmetijski-zavod.si](https://www.kmetijski-zavod.si)

- **Abest-Horizon** – Meylan, France  
  [www.dia4s.fr](https://www.dia4s.fr)

- **Federal Institute for Geology, Geophysics, Climatology and Meteorology (GeoSphere Austria)** – Vienna, Austria  
  [www.geosphere.at](https://www.geosphere.at)

- **Ludwig-Maximilians-Universität München (LMU)** – Munich, Germany  
  [www.geographie.uni-muenchen.de](https://www.geographie.uni-muenchen.de)

- **Functional Center and Planning of Aosta Valley (EWS Bureau)** – Aosta, Italy  
  [cf.regione.vda.it](https://cf.regione.vda.it)

- **CIMA Research Foundation** – Savona, Italy  
  [www.cimafoundation.org](https://www.cimafoundation.org)

- **National Research Institute for Agriculture, Food and Environment (INRAE LESSEM)** – Saint-Martin-d’Hères, France  
  [www.lessem.fr](https://www.lessem.fr)

- **Technical University of Munich (TUM)** – Munich, Germany  
  [www.cee.ed.tum.de](https://www.cee.ed.tum.de)

- **Technical University of Vienna (TU Wien)** – Vienna, Austria  
  [www.tuwien.at/mg/geo](https://www.tuwien.at/mg/geo)

- **University of Ljubljana** – Ljubljana, Slovenia  
  [www.uni-lj.si](https://www.uni-lj.si) | [www.bf.uni-lj.si/en](https://www.bf.uni-lj.si/en)

### Keywords

Trockenheit im Alpenraum, Dürre in den Alpen, Observatorium für Dürre in den Alpen  
Siccità nelle Alpi, Osservatorio della siccità nelle Alpi  
Suša v Alpah, Observatorij za sušo v Alpah  
Sécheresse dans l'espace alpin, Sécheresse dans les Alpes, Observatoire de la sécheresse dans les Alpes  
Drought in the Alps, Drought observatory in the Alps
